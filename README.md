...
首屏是全幅视频构图，视频层固定在文字与界面之下。

**桌面端**：视频随鼠标横向移动擦洗定位（seek 串行防抖），定格在 12% 帧作为初始画面。

**手机端**：默认不加载视频，只显示渐变画布加一个「播放背景」按钮，点击后才挂载视频并以静音循环方式播放。这样扫码打开不会立刻消耗整段视频流量，弱网下首屏也不会卡。判断依据是 `matchMedia('(max-width: 860px)')`。

视频加载失败或 8 秒拿不到元数据时自动回退到渐变画布（`HeroCanvas`：四个色团利萨如轨迹，26 秒精确循环，内部低分辨率拉伸加模糊，开销与视口无关）。

想换成真视频：把 mp4 丢进 `public/`，然后在 `resumeData.js` 里填
...
- **首屏重排**：头像是在桌面端绝对定位、与字标同高的圆形。手机上改为回到文档流（`.hero` 本身是 flex column，头像块在 DOM 中位于字标之前，改静态后自然落在字标上方），尺寸固定 64px 与字体度量解耦。简介的左内边距从 `shell-pad + portrait-inset`（约 68px）收回常规内边距，375px 下正文可用宽度由约 291px 恢复到约 339px
...
| 视差 | 头像块下沉、字标上漂（scrub），仅桌面端；手机上头像已在文档流中，位移会压到字标 |
| 数据 | 进入视口后数字滚动 |
