...
/**
 * Grainient — 来自 React Bits（reactbits.dev）的开源 WebGL 渐变背景。
 * 原始实现对本项目的两处调整：
 * 1. 新增 paused 开关，配合 prefers-reduced-motion 只渲染静态一帧。
 * 2. 采样上限从 2 降到 1.5，全屏背景在高清屏上不必跑满 2 倍像素。
 */
...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      alpha: true,
      antialias: false,
      dpr: Math.min(window.devicePixelRatio || 1, isNarrow ? 1 : 1.5),
    });
    ...
    ctxMap.set(container, { renderer, program, mesh, tryStart, tryStop, renderOnce });
  ...
  // Effect 3: 减弱动效偏好切换时停/启循环，静态时补一帧
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const ctx = ctxMap.get(container);
